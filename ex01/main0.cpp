...
int Brain_ocf_test(void) {
  ...
  for (size_t i = 0; i < 10; i++) {
    std::string idea1 = Brain1.getIdea(i);
    std::string idea3 = Brain3.getIdea(i);
    if (idea1 == idea3)
      idea_ok = 1;
    else {
      {
...
#endif
  if (idea_ok)
    std::cout << " [Idea Identity Confirmed!]" << std::endl;
  else
    std::cout << " [Idea NOT MATCH!]" << std::endl;
  return (idea_ok);
}
